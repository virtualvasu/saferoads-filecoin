...
export default function ContractSubmissionStep({ pdfCID, contract, walletAddress, onNext, onBack }: ContractSubmissionStepProps) {
  ...
  const handleSubmitToContract = async () => {
    ...
    try {
      // Call the contract with the IPFS URL as the description
      const tx = await contract.reportIncident(pdfCID);
      const receipt = await tx.wait();

      // Parse the IncidentReported event from the transaction logs
      let incidentData = null;
      
      if (receipt.logs && receipt.logs.length > 0) {
        // Try to parse the event from the logs
        try {
          const parsedLog = contract.interface.parseLog(receipt.logs[0]);
          if (parsedLog && parsedLog.name === 'IncidentReported') {
            incidentData = {
              id: parsedLog.args[0].toString(),
              description: parsedLog.args[1],
              reportedBy: parsedLog.args[2],
              timestamp: new Date(Number(parsedLog.args[3]) * 1000).toLocaleString(),
              txHash: tx.hash,
