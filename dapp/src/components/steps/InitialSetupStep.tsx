...
import { Wallet, Cloud, ArrowRight, CheckCircle, AlertTriangle, Shield, Network } from 'lucide-react';
...
import { ensureCorrectNetwork, DEFAULT_NETWORK, isCorrectNetwork } from '../../lib/network';
...
export default function InitialSetupStep({ onNext }: InitialSetupStepProps) {
  ...
  const connectWallet = async () => {
    ...
    try {
      ...
      if (!networkSwitched) {
        alert(`Please switch to ${DEFAULT_NETWORK.name} (Chain ID: ${DEFAULT_NETWORK.chainId}) to continue.`);
  ...
  const canContinue = walletConnected && networkCorrect && isStorachaConnected && contract && storachaCredentials;
        ...
        <div className={`p-4 rounded-lg border-2 transition-colors ${
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">
                {networkCorrect ? `Connected to ${DEFAULT_NETWORK.name}` : `Please switch to ${DEFAULT_NETWORK.name}`}
              </p>
            </div>
          </div>
        </div>
          ...
          {!walletConnected ? (
            <div>
              <p className="text-gray-600 mb-4">
                Connect your MetaMask wallet to sign transactions and interact with the Hedera blockchain.
              </p>
              <button
